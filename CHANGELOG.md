...
+ [新增] 画布支持切换到剪辑模式，当前画布的视频和音频会进入 AiCut 素材箱，并可继续用右侧 Agent 组织时间线。
...
+ [调整] 新建配置默认使用 LovBrowser LLM API，并预置图片、视频、文本和音频模型。
+ [调整] Canvas Agent 内置 Codex 升级至 0.154.0，并将包版本提升至 0.6.1。
+ [优化] 首页改为 LovBrowser 风格，并提供视频工作流入口与品牌域名部署配置。
