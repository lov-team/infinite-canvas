...
async function createPluginVideoTask(config: AiConfig, model: string, script: string, prompt: string, references: ReferenceImage[], options?: VideoMediaOptions): Promise<VideoGenerationTask> {
    ...
    const result = videoPluginResult(
        await runModelPlugin({
            ...
            audios,
            params: {
                seconds: isH3Model(model) ? clampH3Seconds(config.videoSeconds) : normalizeVideoSeconds(config.videoSeconds),
                size: isH3Model(model) ? h3Resolution(config.vquality) : normalizeVideoSize(config.size, config.vquality) || videoAspectRatio(config.size),
                resolution: isH3Model(model) ? h3Resolution(config.vquality) : normalizeVideoResolution(config.vquality),
                ratio: isH3Model(model) ? h3Ratio(config.size) : videoAspectRatio(config.size),
