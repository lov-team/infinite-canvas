...
export const toolInputSchemas = {
    ...
    canvas_set_workspace_mode: z.object({ mode: z.enum(["canvas", "edit"]) }),
    editor_get_state: z.object({}).passthrough(),
    editor_apply_ops: z.object({
        ops: z.array(z.discriminatedUnion("type", [
            z.object({ type: z.literal("add_clip"), sourceId: z.string().optional(), nodeId: z.string().optional(), mode: z.enum(["append", "overlay"]).optional(), startMs: z.number().optional(), in: z.number().optional(), out: z.number().optional() }),
            z.object({ type: z.literal("move_clip"), clipId: z.string(), start: z.number().optional(), trackId: z.string().optional(), newTrack: z.boolean().optional() }),
            z.object({ type: z.literal("split"), timeMs: z.number().optional(), clipId: z.string().optional() }),
            z.object({ type: z.literal("trim_left"), timeMs: z.number().optional() }),
            z.object({ type: z.literal("trim_right"), timeMs: z.number().optional() }),
            z.object({ type: z.literal("remove_clip"), clipId: z.string() }),
            z.object({ type: z.literal("set_selection"), clipId: z.string().nullable().optional() }),
        ])),
    }),
...
export const toolDescriptions: Record<ToolName, string> = {
    ...
    canvas_get_state: "读取当前网页画布的节点、连线、选区和视口；若在剪辑模式还会包含 editor 时间线摘要。",
    ...
    canvas_set_workspace_mode: "切换当前画布工作区。mode=canvas 显示无限画布，mode=edit 进入 AiCut 剪辑器；剪辑模式下画布视频/音频会进入时间线素材箱。",
    editor_get_state: "读取当前画布剪辑器的素材箱、轨道、片段、时长和选中 clip。不在剪辑模式时返回 workspaceMode=canvas。组织时间线前先调用本工具。",
    editor_apply_ops: "批量操作当前画布的 AiCut 时间线。ops 支持 add_clip（可用 nodeId 或 sourceId）、move_clip、split、trim_left、trim_right、remove_clip、set_selection。不在剪辑模式时会自动切换。",
